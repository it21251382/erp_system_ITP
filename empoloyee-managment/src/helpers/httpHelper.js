...
const apiPrefix = process.env.REACT_APP_API_LINK_PREFIX;
...
export const POST = async (url, data, contentType = 'application/json', useHeaders = true) => {
	try {
		const axiosClient = useHeaders ? axiosWithHeaders() : axiosWithoutHeaders();
...
export const PUT = async (url, data, contentType = 'application/json', useHeaders = true) => {
	try {
		const axiosClient = useHeaders ? axiosWithHeaders() : axiosWithoutHeaders();
		const headers = { 'Content-Type': contentType };
